...
int SLang_init_tty (int abort_char, int no_flow_control, int opost)
{
  ...
  ShellConsole = GetStdHandle(STD_OUTPUT_HANDLE);
  /*ShellConsole = GetStdHandle(STD_OUTPUT_HANDLE);*/
...
/*  if (SetConsoleMode(hStdin, 0) == FALSE) {   Siegward */
  if (SetConsoleMode(hStdin, ENABLE_MOUSE_INPUT) == FALSE) {
    return -1; /* failure */
  }
