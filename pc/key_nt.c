...
#define MOUSE_NEW
//#define SIEGWARD_TEST
#ifdef SIEGWARD_TEST
void test_mouse (int use_printf)
{
  int loop = 1;
  int MAX_COORDINATES = 20;
  int coordinate_counter = 0;
  COORD mouse_coordinates[MAX_COORDINATES];
  memset(mouse_coordinates,0,sizeof(mouse_coordinates));

	while (loop)
  	{
  	INPUT_RECORD ir;
    DWORD number_of_records_read = 0;
    BOOL ok = PeekConsoleInput(hConsoleInput,&ir,1,&number_of_records_read);
    	if (!ok)
      	{
    ...
    switch (ir.EventType)
    	{
      case KEY_EVENT:
        ReadConsoleInput(hConsoleInput,&ir,1,&number_of_records_read);
        if (use_printf)
      		printf("Key = '%c'\n",ir.Event.KeyEvent.uChar.AsciiChar);
        if (ir.Event.KeyEvent.uChar.AsciiChar == 'n')
        	loop = 0;
        break;
                                                                                        break;
      case MOUSE_EVENT:
        ...
        if (use_printf)
        	...
        	{
        	mouse_coordinates[coordinate_counter++] =
					                                ir.Event.MouseEvent.dwMousePosition;
          if (coordinate_counter == MAX_COORDINATES)
          	loop = 0;
          }
        break;
      }
...
int get_win_event (Gpm_Event *event, int no_delay)
{
    INPUT_RECORD ir;				/* Input record */
    DWORD		 dw;				/* number of records actually read */
    int			 ch, vkcode, j;
    ...
    switch (ir.Event.MouseEvent.dwButtonState)
      ...
      if (event->buttons)
      	{
				event->type = GPM_DOWN;
        clicks = ir.Event.MouseEvent.dwEventFlags == DOUBLE_CLICK ? 2 : 1;
        }
      else
      	{
        event->type = 0;
				event->type = GPM_UP | (clicks == 2 ? GPM_DOUBLE : GPM_SINGLE);
        }
			break;
		    case MOUSE_MOVED:
			event->type = GPM_MOVE;
...
int get_event (Gpm_Event *event, int redo_event, int block)
{
    ...
    if (block) {
    	while (1) {
				/* Try to get an event */
				c = get_win_event (event, 0);
				if (c != ERR)
	    		break;
    		}
      }
    else
    	c = get_win_event (event, 1);
