...
{
    int status = 0;
#ifdef USE_INTERNAL_SHELL
    internal_shell(command);
#else // USE_INTERNAL_SHELL
    ...
    if (command) {
	  if (!access(command, 0)) {
       ...
       spawnlp (P_WAIT, shell, shell, (char *) 0);; // $$ neu
      }
    else
